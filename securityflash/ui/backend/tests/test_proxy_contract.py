...
def test_no_models_directory():
    ...
    assert not os.path.exists("models/"), "models/ directory must not exist (moved to docs/legacy/)"


def test_no_alembic_directory():
    """V2 BFF must NOT have alembic/ directory (no migrations)."""
    assert not os.path.exists("alembic/"), "alembic/ directory must not exist (moved to docs/legacy/)"


def test_no_database_file():
    """V2 BFF must NOT have database.py (no DB connection)."""
    assert not os.path.exists("database.py"), "database.py must not exist (moved to docs/legacy/)"


def test_proxy_utility_exists():
    """V2 BFF must have proxy utility."""
    assert os.path.exists("api/proxy.py"), "api/proxy.py must exist"
    
    with open("api/proxy.py", "r") as f:
        content = f.read()
...
def test_no_audit_log_service():
    ...
    assert not os.path.exists("services/audit_log_service.py"), "audit_log_service.py must be in legacy/"
    assert not os.path.exists("services/audit_service.py"), "audit_service.py must be in legacy/"


def test_legacy_directory_exists():
    """Legacy DB files must be in docs/legacy/ with warning."""
    assert os.path.exists("docs/legacy/"), "docs/legacy/ must exist"
    assert os.path.exists("docs/legacy/README.md"), "docs/legacy/README.md must exist"
    
    with open("docs/legacy/README.md", "r") as f:
        content = f.read()
